...
releaseLevelFallbackEl.value = localStorage.getItem(STORAGE_KEYS.releaseLevelFallback) || "disabled";
...
async function loadSourcePreferences() {
	...
	try {
		savedPreferences = await window.electronAPI.getSourcePreferences()
	} catch (error) {
		for (const [key, control] of SOURCE_PREFERENCE_CONTROLS) {
			control.checked = localStorage.getItem(key) === "true"
		}
		statusEl.textContent = `Failed to load source preferences: ${getUiErrorMessage(error)}`;
		showFiles();
		return
	}
	for (const [key, control] of SOURCE_PREFERENCE_CONTROLS) {
		if (Object.prototype.hasOwnProperty.call(savedPreferences, key)) {
			control.checked = savedPreferences[key] === true;
		...
		control.checked = legacyValue === "true";
		try {
			await window.electronAPI.setSourcePreference(key, control.checked);
			localStorage.removeItem(key)
		} catch (error) {
			statusEl.textContent = `Failed to migrate source preferences: ${getUiErrorMessage(error)}`
		}
	}
	showFiles()
}
...
async function saveSourcePreferenceAndReload(storageKey, checked) {
	...
	try {
		await window.electronAPI.setSourcePreference(storageKey, checked);
