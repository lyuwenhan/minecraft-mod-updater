...
const contentFilterEl = document.getElementById("content-filter");
...
function clearTargetLookups() {
	for (const item of importedFiles) {
		item.targetLookup = null;
		item.download = null;
		item.downloadError = ""
	}
}

function clearTargetLookupsAndRestoreSearchAction() {
	clearTargetLookups();
...
function targetFound(item) {
	return Boolean(item.targetLookup?.providers?.some(provider => provider.available))
}

function hasUnqueriedTargetItems() {
	return importedFiles.some(item => originalFound(item) && !item.targetLookup)
}

function syncContentFilterOptions() {
	const hideTargetOptions = hasUnqueriedTargetItems();
	for (const option of contentFilterEl.options) {
		if (option.value === "all-found" || option.value === "original-only") option.hidden = hideTargetOptions
	}
	if (hideTargetOptions && (contentFilterEl.value === "all-found" || contentFilterEl.value === "original-only")) contentFilterEl.value = "all"
}
...
function targetVersionRecords(result, item) {
	if (!result.modrinthDownload) return [];
	return [{
		provider: "modrinth",
		available: true,
		modVersion: normalizedVersionId(result.modrinthDownload.versionName || ""),
		siteName: item.modrinth?.project?.title || "",
		url: providerSiteUrl(item)
	}]
}
...
function passesContentFilter(item) {
	const value = contentFilterEl.value;
	const hasOriginal = originalFound(item);
	const targetChecked = Boolean(item.targetLookup);
	if (value === "all-found") return hasOriginal && (!targetChecked || targetFound(item));
	if (value === "original-only") return hasOriginal && (!targetChecked || !targetFound(item));
...
function showFiles() {
	...
	for (const {
		...
		if (originalFound(item) && item.targetLookup) card.append(createProviderRow(item.targetLookup.providers, "Target version"));
...
function saveTargetPreferenceAndClearTargetLookup(storageKey, value) {
	localStorage.setItem(storageKey, value);
	clearTargetLookupsAndRestoreSearchAction();
	showFiles()
}
...
function mergeTargetState(reloaded, previousItems) {
	const stateByPath = new Map(previousItems.map(item => [item.path, {
		...
		targetLookup: item.targetLookup || null
	}]));
...
targetVersionSelectEl.addEventListener("change", () => {
	...
	saveTargetPreferenceAndClearTargetLookup(STORAGE_KEYS.targetVersion, selectedTargetVersion())
});
targetVersionEl.addEventListener("input", () => saveTargetPreferenceAndClearTargetLookup(STORAGE_KEYS.targetVersion, selectedTargetVersion()));
targetLoaderEl.addEventListener("change", () => {
	setCustomInputVisibility(targetLoaderCustomEl, targetLoaderEl.value === "other");
	saveTargetPreferenceAndClearTargetLookup(STORAGE_KEYS.targetLoader, selectedTargetLoader())
});
targetLoaderCustomEl.addEventListener("input", () => saveTargetPreferenceAndClearTargetLookup(STORAGE_KEYS.targetLoader, selectedTargetLoader()));
minimumReleaseEl.addEventListener("change", () => saveTargetPreferenceAndClearTargetLookup(STORAGE_KEYS.minimumRelease, minimumReleaseEl.value));
searchQueryEl.addEventListener("input", showFiles);
contentFilterEl.addEventListener("change", showFiles);
async function importPaths(paths) {
	...
	try {
		...
		importedFiles.push(...filterNewImportedItems(added));
		restoreTargetSearchAction();
		showFiles();
...
async function refreshDownloadAvailability({
	...
	const indexedItems = importedFiles.map((item, index) => ({
		item,
		index
	})).filter(({
		item
	...
	try {
		...
		results.forEach((result, resultIndex) => {
			...
			importedFiles[index].targetLookup = {
				providers: targetVersionRecords(result, importedFiles[index])
			};
			importedFiles[index].downloadError = result.modrinthError ? `Modrinth: ${result.modrinthError}` : ""
		});
		setTargetAction("download");
		showFiles();
		const count = results.filter(result => result.download).length;
...
async function downloadTargetVersion() {
	...
	if (targetAction !== "download" || !targetItems.length || targetItems.some(item => !item.targetLookup)) {
