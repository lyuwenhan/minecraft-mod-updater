...
const checkUpdatesEl = document.getElementById("check-updates");
...
const releaseLevelFallbackEl = document.getElementById("release-level-fallback");
const disableLyuwenhanEl = document.getElementById("disable-lyuwenhan");
const disableModrinthEl = document.getElementById("disable-modrinth");
const disableCurseForgeEl = document.getElementById("disable-curseforge");
...
const STORAGE_KEYS = {
	...
	releaseLevelFallback: "release-level-fallback",
	disableLyuwenhan: "disable-lyuwenhan",
	disableModrinth: "disable-modrinth",
	disableCurseForge: "disable-curseforge"
};
let importedFiles = [];
...
let sourcePreferenceReloadQueued = false;
let sourcePreferenceReloadActive = false;
let targetPreferenceRefreshQueued = false;
let targetPreferenceRefreshActive = false;
const SETTING_DEFAULTS = {
	[STORAGE_KEYS.targetVersion]: "",
	[STORAGE_KEYS.targetLoader]: "fabric",
	[STORAGE_KEYS.minimumRelease]: "beta",
	[STORAGE_KEYS.releaseLevelFallback]: "disabled",
	[STORAGE_KEYS.disableLyuwenhan]: false,
	[STORAGE_KEYS.disableModrinth]: false,
	[STORAGE_KEYS.disableCurseForge]: false
};
...
const SOURCE_PREFERENCE_CONTROLS = new Map([
	[STORAGE_KEYS.disableLyuwenhan, disableLyuwenhanEl],
	[STORAGE_KEYS.disableModrinth, disableModrinthEl],
	[STORAGE_KEYS.disableCurseForge, disableCurseForgeEl]
]);
async function loadSettings() {
	try {
		return await window.electronAPI.getSettings()
	} catch (error) {
		statusEl.textContent = `Failed to load settings: ${getUiErrorMessage(error)}`;
		return {}
	}
}

function settingValue(settings, key) {
	return Object.prototype.hasOwnProperty.call(settings, key) ? settings[key] : SETTING_DEFAULTS[key]
}

function applySourcePreferenceSettings(settings) {
	for (const [key, control] of SOURCE_PREFERENCE_CONTROLS) {
		control.checked = settingValue(settings, key) === true
	}
}
async function initializeSettings() {
	const settings = await loadSettings();
	const storedTargetVersion = settingValue(settings, STORAGE_KEYS.targetVersion);
	minimumReleaseEl.value = settingValue(settings, STORAGE_KEYS.minimumRelease);
	releaseLevelFallbackEl.value = settingValue(settings, STORAGE_KEYS.releaseLevelFallback);
	initializeTargetVersionControl(storedTargetVersion);
	initializeTargetLoaderControl(settingValue(settings, STORAGE_KEYS.targetLoader));
	applySourcePreferenceSettings(settings);
	showFiles();
	await loadGameVersions(storedTargetVersion)
}

function sourcePreferences() {
	return {
		disableLyuwenhan: disableLyuwenhanEl.checked,
		disableModrinth: disableModrinthEl.checked,
		disableCurseForge: disableCurseForgeEl.checked
	}
}
...
function setTargetVersionOptions(versions, preferredValue) {
	...
	if (currentValue) {
		...
		targetVersionEl.value = "";
		void saveSetting(STORAGE_KEYS.targetVersion, selectedTargetVersion())
	} else {
...
function filterLookupErrors(errors) {
	const {
		disableLyuwenhan,
		disableModrinth,
		disableCurseForge
	} = sourcePreferences();
	return (errors || []).filter(error => !(disableLyuwenhan && String(error).startsWith("Lyuwenhan Extensions:")) && !(disableModrinth && String(error).startsWith("Modrinth:")) && !(disableCurseForge && String(error).startsWith("CurseForge:")))
}

function applySourcePreferences(item) {
	const {
		disableLyuwenhan,
		disableModrinth,
		disableCurseForge
	...
	return {
		...item,
		lyuwenhanExtensions: disableLyuwenhan ? null : item.lyuwenhanExtensions,
		modrinth: disableModrinth ? null : item.modrinth,
		curseforge: disableCurseForge ? null : item.curseforge,
		modrinthDownload: disableModrinth ? null : item.modrinthDownload,
		curseforgeDownload: disableCurseForge ? null : item.curseforgeDownload,
		lookupErrors: filterLookupErrors(item.lookupErrors)
	}
}

function applySourcePreferencesToItems(items) {
	return (items || []).map(applySourcePreferences)
}

function filterNewImportedItems(items) {
	const accepted = [];
	for (const rawItem of items) {
		const item = applySourcePreferences(rawItem);
...
function preferences() {
	return {
		...
		releaseLevelFallback: releaseLevelFallbackEl.value === "allow-lower"
	}
}
...
function setBusy(value, message = "") {
	...
	if (!value && sourcePreferenceReloadQueued) {
		void runQueuedSourcePreferenceReload()
	}
}
...
function importedFoundCount() {
	return importedFiles.map(applySourcePreferences).filter(originalFound).length
}
...
function originalProviderRecords(item) {
	...
	if (!disableLyuwenhan && disableModrinth && disableCurseForge) {
		return [{
			provider: "lyuwenhan",
			iconClass: "provider-icon-lyuwenhan",
			available: false,
...
function visibleFiles() {
	...
	importedFiles.forEach((item, originalIndex) => {
		const displayItem = applySourcePreferences(item);
		if (!passesContentFilter(displayItem)) {
			return
		}
		const displayName = displayNameForItem(displayItem);
		let matchKind = "none";
		let indices = [];
		if (query) {
...
function createProviderIcon({
	...
	if (siteName && modVersion) {
		icon.title = siteName + "|" + modVersion
	}
	if (available && url) {
		icon.href = url;
...
async function saveSetting(storageKey, value) {
	await window.electronAPI.setSetting(storageKey, value)
}

function queueTargetPreferenceRefresh() {
	targetPreferenceRefreshQueued = true;
	void runQueuedTargetPreferenceRefresh()
}
async function runQueuedTargetPreferenceRefresh() {
	if (targetPreferenceRefreshActive || busy || !targetPreferenceRefreshQueued) {
		return
	}
	if (!importedFiles.length) {
		targetPreferenceRefreshQueued = false;
		return
	}
	targetPreferenceRefreshActive = true;
	try {
		while (targetPreferenceRefreshQueued && !busy) {
			targetPreferenceRefreshQueued = false;
		...
		targetPreferenceRefreshActive = false;
		if (targetPreferenceRefreshQueued && !busy) {
			void runQueuedTargetPreferenceRefresh()
		}
	}
}
async function saveTargetPreferenceAndClearTargetSearchResults(storageKey, value) {
	const shouldRefresh = targetSearched || targetPreferenceRefreshActive || targetPreferenceRefreshQueued;
	try {
		await saveSetting(storageKey, value)
	} catch (error) {
		statusEl.textContent = `Failed to save setting: ${getUiErrorMessage(error)}`;
		showFiles();
		return
	}
	clearTargetSearchResultsAndRestoreSearchAction();
	showFiles();
	if (shouldRefresh && importedFiles.length) {
		queueTargetPreferenceRefresh()
	}
}

function mergeTargetState(reloaded, previousItems) {
	const stateBySha1 = new Map(previousItems.map(item => [item.sha1, {
	...
	const reloadedBySha1 = new Map(reloaded.map(item => [item.sha1, item]));
	return previousItems.map(item => ({
		...reloadedBySha1.get(item.sha1) || item,
		...stateBySha1.get(item.sha1)
	}))
}

function lookupErrorMatchesDisabledSource(error, disabledSources) {
	const message = String(error);
	return Boolean(disabledSources.disableLyuwenhan && message.startsWith("Lyuwenhan Extensions:") || disabledSources.disableModrinth && message.startsWith("Modrinth:") || disabledSources.disableCurseForge && message.startsWith("CurseForge:"))
}

function mergeLookupErrorsForSourcePreferenceReload(currentErrors, previousErrors, disabledSources) {
	const activeErrors = (currentErrors || []).filter(error => !lookupErrorMatchesDisabledSource(error, disabledSources));
	const cachedDisabledErrors = (previousErrors || []).filter(error => lookupErrorMatchesDisabledSource(error, disabledSources));
	return [...activeErrors, ...cachedDisabledErrors]
}

function mergeSourcePreferenceReloadState(reloaded, previousItems, disabledSources) {
	const previousBySha1 = new Map(previousItems.map(item => [item.sha1, item]));
	return mergeTargetState(reloaded.map(item => {
		const previous = previousBySha1.get(item.sha1) || {};
		return {
			...item,
			lyuwenhanExtensions: disabledSources.disableLyuwenhan ? previous.lyuwenhanExtensions || item.lyuwenhanExtensions : item.lyuwenhanExtensions,
			modrinth: disabledSources.disableModrinth ? previous.modrinth || item.modrinth : item.modrinth,
			curseforge: disabledSources.disableCurseForge ? previous.curseforge || item.curseforge : item.curseforge,
			lookupErrors: mergeLookupErrorsForSourcePreferenceReload(item.lookupErrors, previous.lookupErrors, disabledSources)
		}
	}), previousItems)
}

function queueSourcePreferenceReload() {
	sourcePreferenceReloadQueued = true;
	void runQueuedSourcePreferenceReload()
}
async function runQueuedSourcePreferenceReload() {
	if (sourcePreferenceReloadActive || busy || !sourcePreferenceReloadQueued) {
		return
	}
	if (!importedFiles.length) {
		sourcePreferenceReloadQueued = false;
		showFiles();
		return
	}
	sourcePreferenceReloadActive = true;
	try {
		while (sourcePreferenceReloadQueued && !busy) {
			sourcePreferenceReloadQueued = false;
			setBusy(true, `Refreshing status for ${importedFiles.length} files`);
			try {
				const disabledSources = sourcePreferences();
				const previousItems = importedFiles;
				const reloaded = await window.electronAPI.refreshSources({
					items: importedFiles,
					disabledSources
				});
				importedFiles = mergeSourcePreferenceReloadState(reloaded, previousItems, disabledSources);
				rebuildImportCaches();
				showFiles();
		...
		sourcePreferenceReloadActive = false;
		if (sourcePreferenceReloadQueued) {
			void runQueuedSourcePreferenceReload()
		}
	}
}
async function saveSourcePreferenceAndReload(storageKey, checked) {
	const control = SOURCE_PREFERENCE_CONTROLS.get(storageKey);
	try {
		await saveSetting(storageKey, checked)
	} catch (error) {
		if (control) {
			control.checked = !checked
		}
		statusEl.textContent = `Failed to save source preference: ${getUiErrorMessage(error)}`;
		showFiles();
		return
	}
	queueSourcePreferenceReload()
}
...
releaseLevelFallbackEl.addEventListener("change", () => {
	saveTargetPreferenceAndClearTargetSearchResults(STORAGE_KEYS.releaseLevelFallback, releaseLevelFallbackEl.value)
});
disableLyuwenhanEl.addEventListener("change", () => saveSourcePreferenceAndReload(STORAGE_KEYS.disableLyuwenhan, disableLyuwenhanEl.checked));
disableModrinthEl.addEventListener("change", () => saveSourcePreferenceAndReload(STORAGE_KEYS.disableModrinth, disableModrinthEl.checked));
disableCurseForgeEl.addEventListener("change", () => saveSourcePreferenceAndReload(STORAGE_KEYS.disableCurseForge, disableCurseForgeEl.checked));
...
async function importPaths(paths) {
	...
	try {
		const response = await window.electronAPI.importMods({
			...
			disabledSources: sourcePreferences()
		});
...
async function refreshDownloadAvailability({
	...
	try {
		...
		results.forEach((result, resultIndex) => {
			...
			importedFiles[index].modrinthDownload = disableModrinthEl.checked ? null : result.modrinthDownload || null;
			importedFiles[index].curseforgeDownload = disableCurseForgeEl.checked ? null : result.curseforgeDownload || null;
