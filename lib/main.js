...
function setTargetVersionOptions(versions, preferredValue) {
	...
	for (const version of versions) {
		if (typeof version !== "string" || !version || seen.has(version)) continue;
		seen.add(version);
...
function removeImportedFileAt(index) {
	...
	if (index !== lastIndex) importedFiles[index] = importedFiles[lastIndex];
	importedFiles.pop()
}
...
function originalFound(item) {
	return Boolean(item.lyuwenhanExtensions || item.modrinth)
}
...
function providerSiteUrl(item) {
	if (item.lyuwenhanExtensions) return firstLyuwenhanExtensionsLink(item) || LYUWENHAN_EXTENSIONS_URL;
...
function displayNameForItem(item) {
	return item.lyuwenhanExtensions?.displayName || item.modrinth?.project?.title || ""
}

function descriptionForItem(item) {
	return item.lyuwenhanExtensions?.description || item.modrinth?.project?.description || ""
}

function iconUrlForItem(item) {
	if (item.lyuwenhanExtensions) return item.lyuwenhanExtensions.hasIcon ? item.lyuwenhanExtensions.iconUrl || "" : "";
...
function createProviderRow(providers, label = "") {
	...
	for (const provider of providers || []) row.append(createProviderIcon(provider));
	return row
}
...
async function refreshDownloadAvailability({
	...
	try {
		...
		results.forEach((result, resultIndex) => {
			...
			importedFiles[index].downloadError = result.downloadError || (result.modrinthError ? `Modrinth: ${result.modrinthError}` : "")
		});
		targetSearched = true;
		setTargetAction("download");
		showFiles();
